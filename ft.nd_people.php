  ...
  /**
	...
	private function _include_css()
	{
        if ( !$this->cache['css'] )
        {
            $this->EE->cp->add_to_head('<style type="text/css">
                ...
                .vz_address_phone_cell { float:left; width:48%; }
                .vz_address_fax_cell { float:right; width:48%; }
                .vz_address_country_cell { float:left; width: 98.5%; }
                .vz_address_email_address_cell { float:left; width: 101%; }
            </style>');
    ...
    function replace_tag($address, $params=array(), $tagdata=FALSE)
    {
        ...
        if (!$tagdata) // Single tag
        {
            ...
            {
                case 'inline' :
                    $output = (!empty($address['name']) ? $address['name'].', ' : '')."{$address['street']}, ".($address['street_2'] ? $address['street_2'].', ' : '')."{$address['city']}, {$address['region']} {$address['postal_code']}, {$this->EE->lang->line($address['country'])}, {$address['phone_1']}, {$address['fax']}, {$address['email_address']}";
                    break;
                case 'plain' :
                    $output = "
                ...
                case 'rdfa' :
                    $output = "
                        <div xmlns:v='http://rdf.data-vocabulary.org/#' typeof='v:Address' class='adr' {$wrapper_attr}>
                            ...
                            <div>
                            	<span property='v:tel' class='tel'><span property='v:type' class='type'>Phone:</span>{$address['phone']}</span>
                            </div>
                            <div>
                            	<span property='v:tel' class='tel'><span property='v:type' class='type'>Fax:</span>{$address['fax']}</span>
                            </div>
                            <div property='v:email_address' class='email_address'>{$address['email_address']}</div>
                        </div>";
                    break;
                case 'schema' :
                    $output = "
                ...
                case 'microformat' : default :
                    ...
                    $output .= "
                        <div class='adr' {$wrapper_attr}>
                            ...
                            <div>
                            	<span class='tel'><span class='type'>Phone:</span>{phone}</span>
                            </div>
                            <div>
                            	<span class='tel'><span class='type'>Fax:</span>{fax}</span>
                            </div>
                            <div class='email_address'>{$address['email_address']}</div>
                        </div>";
    ...
    function replace_phone($address, $params=array(), $tagdata=FALSE)
    {
        return $address['phone'];
    }
    function replace_fax($address, $params=array(), $tagdata=FALSE)
    {
        return $address['fax'];
    }
    function replace_email_address($address, $params=array(), $tagdata=FALSE)
    {
    ...
    function replace_map_url($address, $params=array(), $tagdata=FALSE)
    {
        ...
        if (isset($address['phone'])) unset($address['phone']);
        if (isset($address['email_address'])) unset($address['email_address']);
        if (isset($address['fax'])) unset($address['fax']);
